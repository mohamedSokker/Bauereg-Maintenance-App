...
const QrCode = ({ navigation, socket, userID }) => {
  ...
  const checkScan = async (data) => {
    try {
      setIsLoading(true);
      const target = reportData?.filter(
        (datas) =>
          datas.Problem_start_From == fieldsData.Problem_start_From &&
          datas.Equipment === fieldsData.Equipment
      );
      console.log(target);
      const resTime = Math.floor((new Date() - currentTime) / 1000);
      ...
      const url = `${EXPO_PUBLIC_BASE_URL}/api/v1/appMaintMaintenance/${target[0]?.ID}`;
  ...
  const handleCancel = async () => {
    try {
      ...
      const target = reportData?.filter(
        (data) =>
          data.Problem_start_From == fieldsData.Problem_start_From &&
          data.Equipment === fieldsData.Equipment
      );
      const resTime = Math.floor((new Date() - currentTime) / 1000);
      let copyData = { ...target[0] };
